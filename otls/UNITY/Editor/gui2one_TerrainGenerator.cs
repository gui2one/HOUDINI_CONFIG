...
public class gui2one_TerrainGenerator : EditorWindow
{
	...
	//Tree settings
	public int m_treeSpacing = 8; //spacing between trees
	public float m_treeDistance = 30.0f; //The distance at which trees will no longer be drawn
	public float m_treeBillboardDistance = 100.0f; //The distance at which trees meshes will turn into tree billboards
	...
	public int m_treeMaximumFullLODCount = 100; //The maximum number of trees that will be drawn in a certain area. 
	//Detail settings
		...
		for(int x = 0; x < m_terrainSize; x += m_treeSpacing) 
		{
            for (int z = 0; z < m_terrainSize; z += m_treeSpacing) 
			{
				...
				if(frac < 0.3f) //make sure tree are not on steep slopes
				{
					...
					if(noise > 0.0f && ht < m_terrainHeight*0.9f)
					{
